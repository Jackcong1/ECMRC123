...
def precision_recall_f1(prediction , ground_truth):
    ...
    if num_same == 0:
        return 0 , 0 , 0
    p = num_same / len(prediction_tokens)
    r = num_same / len(ground_truth_tokens)  # default r=1
    f1 = (2 * p * r) / (p + r)
    return p , r , f1
...
def metric_all_ground_truths(metric_fn , bleu , rouge , prediction , ground_truths):
    scores_for_ground_truths = []
    a,b,c,n = 1,0.5,0.5,3
    for ground_truth in ground_truths:
        # truth = []
        ...
        score = metric_fn(prediction , ground_truth)
        scores_for_ground_truths.append(score)
    f1_score = max(scores_for_ground_truths)
    score = 0
    ...
    scores = {}
    for ground_truth in ground_truths:
        truths = ''.join(ground_truth)
        pred_dict[question_id] = normalize([prediction])
        ref_dict[question_id] = normalize([truths])

        # bleu_rouge = compute_bleu_rouge(pred_dict , ref_dict)
        # score_bleu.append(bleu_rouge['Bleu-4'])
        # score_rouge.append(bleu_rouge['Rouge-L'])

        if bleu is not None:
            assert set(pred_dict.keys()) == set(ref_dict.keys()) , \
                "missing keys: {}".format(set(ref_dict.keys()) - set(pred_dict.keys()))

            bleu_scores , _ = Bleu(4).compute_score(ref_dict , pred_dict)
            for i , bleu_score in enumerate(bleu_scores):
                scores['Bleu-%d' % (i + 1)] = bleu_score
            score_bleu.append(scores['Bleu-4'])

        if rouge is not None:
            rouge_score , _ = Rouge().compute_score(ref_dict , pred_dict)
            scores['Rouge-L'] = rouge_score
            score_rouge.append(scores['Rouge-L'])

    if bleu is not None:
        score += b*sum(score_bleu)/len(score_bleu)
    if rouge is not None:
        score += c*sum(score_rouge)/len(score_rouge)

    return score/n


def stopwordslist():
    path = r'/home/congyao/stopwords/stopwords.txt'
