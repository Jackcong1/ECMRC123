...
class ON_LSTM(RNNCell):
    ...
    def cumsum(self , x , direction='right'):
        ...
        if direction == 'right':
            # tf.summary.histogram('softmax_x_right' , tf.nn.softmax(x , -1))
            # tf.summary.histogram('gumbel_softmax_x_right' , self.differentiable_gumbel_sample(tf.nn.softmax(x , -1),  temperature=0.1))
            output = tf.cumsum(self.differentiable_gumbel_sample(tf.nn.softmax(x , -1) ,  temperature=0.1) , -1)

            return output
        elif direction == 'left':
            # tf.summary.histogram('softmax_x_left' , tf.nn.softmax(x , -1))
            # tf.summary.histogram('gumbel_softmax_x_left' , self.differentiable_gumbel_sample(tf.nn.softmax(x , -1) ,  temperature=0.1))
            output = 1 - tf.cumsum(self.differentiable_gumbel_sample(tf.nn.softmax(x , -1) ,  temperature=0.1) , -1)
            return output
        # else:
        #     return output

    def differentiable_gumbel_sample(self, logits, temperature):
        noise = tf.random_uniform(tf.shape(logits), seed=11)

        logits_with_noise = logits + noise
        # plt.plot([i for i in range(len(logits))], tf.nn.softmax(logits_with_noise / temperature))
        # plt.show()
