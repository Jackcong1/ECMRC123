...
class Vocab(object):
    ...
    def load_pretrained_embeddings(self, embedding_path):
        ...
        # unknown = np.asarray(rng.normal(size=(self.embed_dim)))
        # trained_embeddings[self.unk_token] = unknown
        ...
        for token in self.token2id.keys():
            if token in trained_embeddings and len(trained_embeddings[token]) > 0:
                try:
                    self.embeddings[self.get_id(token)] = trained_embeddings[token]
                except Exception as e:
                    print(trained_embeddings[token])
                    continue
