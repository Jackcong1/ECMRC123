...
class RCModel(object):
    ...
    def __init__(self , vocab , args):
        ...
        # self.writer = tf.summary.FileWriter("/home/congyao/DuReader-master/data/summary/log" , self.sess.graph)
    ...
    def _build_graph(self):
        ...
        self._embed()
        self._encode()
        self._shared_paramater_encoder()
        self._indenpent_encoder()
    ...
    def _embed(self):
        ...
        with tf.device('/cpu:0') , tf.variable_scope('word_embedding'):
            ...
            #tf.summary.histogram('word_embedding' , self.p_emb)
    ...
    def _encode(self):
        ...
        with tf.variable_scope('passage_encoding'):
            ...
            #tf.summary.histogram('onlstm_encoder' , self.sep_p_encodes)

        with tf.variable_scope('question_encoding'):
            self.sep_q_encodes , _ = rnn('bi-oncell' , self.q_emb , self.q_length , self.hidden_size)

            # self.sep_p_encodes1 = tf.concat([self.p_emb, self.sep_p_encodes], -1)
            # self.sep_q_encodes1 = tf.concat([self.q_emb, self.sep_q_encodes], -1)

            # self.sep_p_encodes = tc.layers.fully_connected(self.sep_p_encodes1, num_outputs=self.hidden_size * 2, activation_fn=tf.nn.elu)
            # self.sep_q_encodes = tc.layers.fully_connected(self.sep_q_encodes1, num_outputs=self.hidden_size * 2, activation_fn=tf.nn.elu)
    ...
    def _shared_paramater_encoder(self):
        with tf.variable_scope('shared_paramater_encoder' , reuse=tf.AUTO_REUSE):
    ...
    def _hybrid_encoder(self):
        with tf.variable_scope('hybrid_encoder'):
    ...
    def _match(self):
        ...
        #tf.summary.histogram('match_p_encodes' , self.match_p_encodes)
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion' , reuse=tf.AUTO_REUSE):
            self.match_p_encodes = tf.concat([self.hybrid_encoding , self.match_p_encodes] , -1)
            ...
            #tf.summary.histogram('fuse' , self.fuse_p_encodes)
    ...
    def _decode(self):
        ...
        #tf.summary.histogram('start' , self.start_probs)
    ...
    def _compute_loss(self):
        ...
        #tf.summary.scalar('start_loss' , self.start_loss)
    ...
    def _train_epoch(self , train_batches , dropout_keep_prob , epoch):
        ...
        for bitx , batch in enumerate(train_batches , 1):
            ...
            #summary_str = self.sess.run(self.merged , feed_dict=feed_dict)
            #self.writer.add_summary(summary_str , epoch * bitx)
        ...
        for epoch in range(1 , epochs + 1):
            ...
            if evaluate:
                ...
                if data.dev_set is not None:
                    ...
                    #tf.summary.scalar('dev_loss' , eval_loss)
    ...
    def evaluate(self , eval_batches , result_dir=None , result_prefix=None , save_full_info=False):
        ...
        for b_itx , batch in enumerate(eval_batches):
            ...
            #                self.writer.add_summary(loss, b_itx)
            total_loss += loss * len(batch['raw_data'])
            total_num += len(batch['raw_data'])
        ...
        if len(ref_answers) > 0:
            ...
            bleu_rouge = None
        # tf.summary.scalar("bleu_rouge" , bleu_rouge)
        ...
        for start_idx in range(passage_len):
            for ans_len in range(self.max_a_len):
                ...
                if end_idx >= passage_len:
                    continue
                prob = (start_probs[start_idx] + wp_score_start[start_idx]) * (
                            end_probs[end_idx] + wp_score_end[end_idx])
                if prob > max_prob:
