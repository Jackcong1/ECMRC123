...
class RCModel(object):
    ...
    def _shared_paramater_encoder(self):
        with tf.variable_scope('shared_paramater_encoder', reuse=tf.AUTO_REUSE):
            self.shared_p_encodes, _ = rnn('bi-lstm', self.sep_p_encodes1, self.p_length, self.hidden_size)
            self.shared_q_encodes, _ = rnn('bi-lstm', self.sep_q_encodes1, self.q_length, self.hidden_size)

    def _indenpent_encoder(self):
        self.indepent_q_encodes, _ = rnn('bi-lstm', self.sep_q_encodes1, self.q_length, self.hidden_size)
    ...
    def _hybrid_encoder(self):
        ...
        sim_matrix2 = tf.matmul(self.sep_p_encodes, self.shared_q_encodes, transpose_b=True)
    ...
    def _fuse(self):
        ...
        with tf.variable_scope('fusion', reuse=tf.AUTO_REUSE):
            self.match_p_encodes = tf.concat([self.hybrid_encoding, self.match_p_encodes, self.p_emb], -1)
